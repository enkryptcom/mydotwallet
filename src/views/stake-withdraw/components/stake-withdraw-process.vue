<template>
  <div v-if="isDone" class="stake-withdraw-process">
    <done-animation />
    <h3>You have successfully<br />withdrawn your assets</h3>
    <base-button title="View details" :action="detailsAction" />
  </div>
  <div v-else-if="isError" class="stake-withdraw-process">
    <error-animation />
    <h3>Something went wrong with your transaction</h3>
    <base-button title="Try again" :action="errorAction" />
  </div>
  <div v-else class="stake-withdraw-process">
    <spinner-animation />
